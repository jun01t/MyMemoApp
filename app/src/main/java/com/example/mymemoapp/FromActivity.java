...
public class FromActivity extends AppCompatActivity {
    ...
    private void deleteMemo() {
        new AlertDialog.Builder(this).setTitle("Delete Memo").setMessage("Are you sure?").setNegativeButton("Cancel", null).setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int witch) {
                Uri uri = ContentUris.withAppendedId(MemoContentProvider.CONTENT_URI,
                        memoId);
    ...
    private void saveMemo() {
        ...
        String updated =
                new SimpleDateFormat("yyyy-MM-dd:kk:mm:ss", Locale.US).format(new Date());
        if (title.isEmpty()) {
            Toast.makeText(FromActivity.this,
                    "Please enter title",
                    Toast.LENGTH_LONG
            ).show();
        } else {
            ...
            if (memoId == 0L) {
                // new memo
                getContentResolver().insert(MemoContentProvider.CONTENT_URI,
                        values);
            } else {
                // updated memo
                Uri uri = ContentUris.withAppendedId(
